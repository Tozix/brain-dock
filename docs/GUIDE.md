...
| Postgres / Qdrant / Redis / Ollama | внутр. сеть (dev: `15432/16333/16379/11434`) | Инфра (Docker Compose); на сервере — без хост-портов |
...
- **Docker** + Docker Compose v2 — этого **достаточно для прод-деплоя на сервере** (образы
  собираются внутри Docker, миграции применяет контейнер `migrate`).
- **Bun** ≥ 1.3 ([bun.sh](https://bun.sh)) — только для **локальной** разработки/сборки; на сервере
  не нужен.
...
`.env.example` ориентирован на **прод** (in-network DNS). Для **dev** (сервисы из bun на хосте)
включите dev-значения в `.env`: `NODE_ENV=development`, раскомментируйте `# dev:` строки URL
(localhost host-порты), `EMBEDDER=deterministic`.

```bash
git clone <repo> && cd brain-dock
cp .env.example .env
# → правим .env под dev (см. выше): NODE_ENV=development, dev-URL'ы, EMBEDDER=deterministic
bun install
bun run infra:up                 # Postgres, Qdrant, Redis, Ollama (+ хост-порты через dev-override)
...
```bash
docker compose --profile app up -d --build     # на сервере — без bun
```
