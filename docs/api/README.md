...
- Покрыты в спецификации (весь REST): auth, api-keys, projects, **repositories** (CRUD +
  `/reindex`), memory/knowledge/documents (включая `PATCH`/`DELETE` item-путей), unified search,
  health/metrics.
...
`/readyz` + Redis `ping`, таймаут 2s каждая; 503 при degraded),
