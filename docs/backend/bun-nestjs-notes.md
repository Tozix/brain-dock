...
**Решение:** запускать процессы, использующие BullMQ, с флагом **`bun --no-addons`** — тогда
загрузка `.node` бросает обычную (ловимую) ошибку вместо паники, и `msgpackr` уходит в JS-fallback.
`apps/workers` (`dev`/`start`-скрипты и Dockerfile CMD) используют `--no-addons`.
Проверено: `bun --no-addons apps/workers/src/bullmq-smoke.ts` → `double(21) = 42`.

> Раньше использовался `postinstall`, удалявший нативный модуль, но это ломало **чистую**
> установку (`bun install` помечал пакет как failed → exit 1) и, как следствие, CI/Docker.
> Флаг `--no-addons` — корректный способ.
