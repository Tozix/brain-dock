...
- **Tools (персистентные, из Qdrant+Postgres):** `list_projects`, `search_code`, `generate_context`,
  `search_everywhere`, `remember`, `search_memory`, `save_knowledge`, `search_knowledge`,
  `save_document`, `search_docs`. Транспорт stateless (`enableJsonResponse`). План [036](../plans/036-remote-mcp-http.md).
- Структурные/граф-tools (`find_*`/architecture/graph) пока **только в локальном stdio-режиме** —
  им нужен серверный индекс символов (отдельный эпик).
